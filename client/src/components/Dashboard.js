...
const Dashboard = () => {
  ...
  // ✅ Function to randomly fluctuate stock prices
  const fluctuatePrices = () => {
    setStocks(prevStocks => {
      return prevStocks.map(stock => {
        // Generate random fluctuation between -2% to +2%
        const fluctuationPercent = (Math.random() - 0.5) * 4;
        const priceChange = stock.price * (fluctuationPercent / 100);
        ...
        return {
          ...
          change: Math.round(newChange * 100) / 100 // Round to 2 decimal places
        };
      });
    });
  };

  // ✅ Set up interval to update prices every 3 seconds
  useEffect(() => {
    const intervalId = setInterval(fluctuatePrices, 3000);
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        <div>
          ...
          <p>Balance: ₹{(user.balance || 10000).toLocaleString('en-IN')}</p>
        </div>
      ...
      <h2 className="stocks-title">Top 54 Traded Stocks</h2>
      <div className="stock-grid">
        {stocks.map((stock) => (
          ...
          >
            <h3>{stock.name}</h3>
            <p className="stock-price">₹{stock.price.toLocaleString('en-IN')}</p>
            <p
            ...
            <div className="price-trend">
              {stock.change > 0.1 ? "↗" : stock.change < -0.1 ? "↘" : "→"}
            </div>
          </div>
        ))}
      </div>
